...
(function () {
  ...
  function setSystem(system) {
    if (system === "v_grade") {
      ...
      if (
        !gradeSelect.value ||
        !gradeSelect.value.toUpperCase().startsWith("V")
      ) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      for (const file of window.getSelectedFiles()) {
        ...
        await fetch("/api/register_upload", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ key, tracker_id: trackerId }),
        });
...
function deleteTracker(trackerId) {
  const confirmed = confirm(
    "Are you sure you want to delete this entire tracker? This will remove all climb data and media files. This action cannot be undone.",
  );
  ...
  if (confirmed) {
    ...
    fetch(`/api/delete_tracker/${trackerId}`, {
      ...
      .catch((error) => {
        ...
        alert(
          "An error occurred while deleting the tracker. Please try again.",
        );
      });
  }
}
